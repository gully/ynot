...
class FPADataset(Dataset):
    ...
    def __init__(self, source_metadata=None, n_ims=None, **kwargs):
        df_all = pd.read_feather(
            "../../zoja/data/J_band_NIRSPEC/calibration_metadata.feather"
        )
        ...
        for i, fn in enumerate(list_of_darks[0:n_ims]):
            self.images[i, :, :] = torch.from_numpy(fits.open(fn)[0].data).to(
                torch.float64
            )
        ...
        device_median = (
            self.images.view((n_ims, 1024 * 1024)).to(device).median(-1).values.cpu()
        )
        ...
        # Combined x'es
        self.xs = torch.stack(
            (torch.from_numpy(det_temps), torch.from_numpy(exp_times)), 1
        ).to(torch.float64)
